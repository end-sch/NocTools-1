...
include ("../libs/db_rbx.php");
	...
	$query = mysqli_query($conn_rbx, "SELECT 
									...
									FROM radacct
									WHERE framedipaddress LIKE '100.64.60.%' AND acctstoptime is NULL
									OR framedipaddress LIKE '100.64.61.%' AND acctstoptime is NULL
									OR framedipaddress LIKE '100.64.62.%' AND acctstoptime is NULL
									OR framedipaddress LIKE '100.64.63.%' AND acctstoptime is NULL
...
$i = 0;
	...
	while($row = mysqli_fetch_assoc($query)) {
		...
		$resposta = $ssh_bng_b->exec('show subscribers address '.$row['framedipaddress'].'  | match pp0. | display json');
