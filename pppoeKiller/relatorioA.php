<?php
	...
	$resposta = $ssh_bng_a->exec('show subscribers client-type pppoe | match pp0. | no-more');
	$ssh_bng_a->exec('exit');
	...
	echo '<strong>Total Juniper BNG-A: </strong>'.$i_junos.'<br>';
	ob_flush();
	flush();
	...
	$query = "SELECT username FROM radius.radacct WHERE nasipaddress = '192.168.109.33' AND acctstoptime is NULL;";
	...
	echo '<strong>Total MKSolutions BNG-A: </strong>'.$i_rbx.'<br>';
	ob_flush();
	flush();
